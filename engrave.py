...
with open('0.000.000_not_modified.MPF') as reader:  # открываем программу для чтения
    ...
    for line in content:  # цикл перебора каждой строки программы
        ...
        if matchM09:
            # берем из строки с М09 циферную часть и сразу делаем это интом
            m09frame = int(matchM09.group(2))

        # ищем строки с G0 с ловлей групп
        matchG0 = re.match(r'(^N)(\d+)(\s)(G0)', line)
        if matchG0:
            # добавляем в лист сразу номер кадра без "N"
            listG0.append(matchG0.group(2))
...
with open('0.000.000_modified.MPF', 'w') as writer:
    for line in content:
        ...
        if 'SUPA' in line:  # замена строк, где используется "SUPA"
            ...
            writer.write(newLine[0]+' '+newLine[1] +
                         ' '+newLine[3]+' '+newLine[4])

        elif ';-------------------------------------' in line:
            writer.write(line)
            writer.write(
                '; -- PROGRAMMA ZARYAZHENA NA VODKU BABU GARMON I LOSOS\n\n')
